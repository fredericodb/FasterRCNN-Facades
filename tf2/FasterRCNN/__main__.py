...
from places365 import placesCNN_predictor
...
def evaluate(model, eval_data = None, num_samples = None, plot = False, print_average_precisions = False):
  if eval_data is None:
    eval_data = voc.Dataset(dir = options.dataset_dir, split = options.test_split, augment = False, shuffle = False)
...
def train(model):
  ...
  print("Initial weights           : %s" % (options.load_from if options.load_from else "Random weights"))
  print("Dataset                   : %s" % options.dataset_dir)
  print("Annotations               : %s" % options.annot_dir)
  print("Training split            : %s" % options.train_split)
  print("Validation split          : %s" % options.val_split)
  print("Testing split             : %s" % options.test_split)
  ...
  print("Results to file           : %s" % ("none" if not options.results_to else options.results_to))
  training_data = voc.Dataset(dir = options.dataset_dir, split = options.train_split, annot_dir=options.annot_dir, feature_pixels = model.backbone_feature_pixels(), augment = not options.no_augment, shuffle = True, cache = options.cache_images)
  val_data = voc.Dataset(dir = options.dataset_dir, split = options.val_split, annot_dir=options.annot_dir, feature_pixels = model.backbone_feature_pixels(), augment = False, shuffle = False, cache = False)
  test_data = voc.Dataset(dir = options.dataset_dir, split = options.test_split, annot_dir=options.annot_dir, feature_pixels = model.backbone_feature_pixels(), augment = False, shuffle = False, cache = False)
  if options.self_train:
    unannotated_data = voc.Dataset(dir = options.dataset_dir, split = options.wtrain_split, annot_dir=options.annot_dir, feature_pixels = model.backbone_feature_pixels(), augment = not options.no_augment, shuffle = True, cache = options.cache_images)
    training_data.join(unannotated_data)
  ...
  print("Evaluating %s model on all samples in '%s'..." % (("best" if options.save_best_to else "final"), options.test_split))  # evaluate final or best model on full dataset
  mean_average_precision = evaluate(
    model = model,
    eval_data = test_data,
  ...
  if options.results_to:
    with open(options.results_to, 'a') as fp:
      fp.write("{}\t{}\t{}\t{}\t{:.6f}\n".format(options.run_number, mean_average_precision, training_data.num_samples, val_data.num_samples, test_data.num_samples))


def _predict(model, image_data, image, show_image, output_path, feature_pixels = 16, only_predict = False):
  anchor_map, anchor_valid_map = anchors.generate_anchor_maps(image_shape = image_data.shape, feature_pixels = feature_pixels)
...
def predict_one(model, url, show_image, output_path):
  ...
  _predict(model = model, image_data = image_data, image = image, show_image = show_image, output_path = output_path, feature_pixels=model.backbone_feature_pixels())
...
def predict_all(model, split):
  ...
  dataset = voc.Dataset(dir = options.dataset_dir, split = split, feature_pixels=model.backbone_feature_pixels(), augment = False, shuffle = False)
  for sample in iter(dataset):
    output_path = os.path.join(dirname, os.path.splitext(os.path.basename(sample.filepath))[0] + ".png")
    _predict(model = model, image_data = sample.image_data, image = sample.image, show_image = False, output_path = output_path, feature_pixels=model.backbone_feature_pixels())

def _predict_to_annot(model, image_data, image, image_file, output_path, feature_pixels = 16):
  scored_boxes_by_class_index = _predict(model = model, image_data = image_data, image = image, show_image = False, output_path = output_path, feature_pixels=model.backbone_feature_pixels(), only_predict=True)

  visualize.write_detections(
    output_path = output_path,
    image = image,
...
def _predict_with_places365(dataset, boxes):
  # Make prediction for weak training set using Places365-CNN and pre-trained Faster R-CNN
  # modifies data_gen_wtrain adding class and bounding box information

  # Predictions from Places365 remapped
  places365_preds = placesCNN_predictor.predict_boxes(dataset, boxes)
  return places365_preds

# Compare, accept or discard images
def compare_accept_discard(places365_preds, frcnn_preds, threshold=0.2):
    boxes = []
    accepted = []
    for idx, img in enumerate(places365_preds):
        p_preds = places365_preds[idx]  # class and boxes predictions from Places365
        f_preds = frcnn_preds[idx]      # class and boxes predictions from Faster R-CNN GSV
        ...
        trythis = False
        for class_id in f_preds:
          if len(f_preds[class_id]) > 0:
            for r in f_preds[class_id]:  # loop for every box of this class in the image
              if p_preds[class_id]:      # if there is a Places365 prediction for this class
                for s in p_preds[class_id]:  # search for the same box in the same class in both classifiers
                  accept = (r[0] == s[0]) and (r[1] == s[1]) and (r[2] == s[2]) and (r[3] == s[3])
                  accept = accept and (r[4] > threshold) and (s[4] > threshold)
                  if accept:
                    preds[class_id].append(r.copy())
                    trythis = True
        preds = {
            1: np.array(preds[1], dtype=np.float64),
            2: np.array(preds[2], dtype=np.float64),
            3: np.array(preds[3], dtype=np.float64),
            4: np.array(preds[4], dtype=np.float64),
            5: np.array(preds[5], dtype=np.float64)
        }
        boxes.append(preds)
        accepted.append(trythis)

    return boxes, accepted


# wtrain_imgs = compare_accept_discard(wtrain_imgs, places365_preds, frcnn_preds, threshold=0.5)
# print('Second pipeline got {} non-annotated images for training.'.format(len(wtrain_imgs)))

def selftrain(model, split):
  dataset = voc.Dataset(dir = options.dataset_dir, split = 'utrain', feature_pixels=model.backbone_feature_pixels(), augment = False, shuffle = False, unannotated=True)
  dirname = "predictions_" + split
  if not os.path.exists(dirname):
    os.makedirs(dirname)
  ...
  frcnn_boxes = []
  for sample in iter(dataset):
    output_path = os.path.join(dirname, os.path.splitext(os.path.basename(sample.filepath))[0] + ".xml")
    scored_boxes_by_class_index = _predict(model = model, image_data = sample.image_data, image = sample.image, show_image = False, output_path = output_path, feature_pixels=model.backbone_feature_pixels(), only_predict=True)
    frcnn_boxes.append(scored_boxes_by_class_index)

  # predict using Places365 with remapoed output
  places365_boxes = _predict_with_places365(dataset = dataset, boxes = frcnn_boxes)

  # combine predictions from model and places365
  combined_boxes, accepted = compare_accept_discard(places365_boxes, frcnn_boxes)

  idx = 0
  wtraining = []
  for sample in iter(dataset):
    output_path = os.path.join(dirname, os.path.splitext(os.path.basename(sample.filepath))[0] + ".xml")
    scored_boxes_by_class_index_final = combined_boxes[idx]
    # save annotation for training
    visualize.write_detections(
      output_path = output_path,
      image = sample.image,
      image_file = sample.filepath,
      scored_boxes_by_class_index = scored_boxes_by_class_index_final,
      class_index_to_name = voc.Dataset.class_index_to_name,
      options = options
    )
    if accepted[idx]:
      wtraining.append(os.path.splitext(os.path.basename(sample.filepath))[0])
    idx += 1
  # write list of pseudo-annotated image files
  setdir = options.dataset_dir + '/ImageSets/Main'
  wtrain_file = os.path.join(setdir, "wtrain.txt")
  file_wtrain = open(wtrain_file, "w")
  for file in wtraining:
    file_wtrain.write(file)
    file_wtrain.write('\n')
  file_wtrain.close()
...
if __name__ == "__main__":
  ...
  group.add_argument("--self-train", action = "store_true", help = "Retrain model")
  ...
  parser.add_argument("--dataset-dir", metavar = "dir", action = "store", default = "gsv_vila_velha", help = "VOC dataset directory")
  parser.add_argument("--train-split", metavar = "name", action = "store", default = "train", help = "Dataset split to use for training")
  parser.add_argument("--val-split", metavar = "name", action = "store", default = "val", help = "Dataset split to use for validation")
  parser.add_argument("--test-split", metavar = "name", action = "store", default = "test", help = "Dataset split to use for testing")
  parser.add_argument("--cache-images", action = "store_true", help = "Cache images during training (requires ample CPU memory)")
  parser.add_argument("--periodic-eval-samples", metavar = "count", action = "store", default = 500, help = "Number of samples to use during evaluation after each epoch")
  ...
  parser.add_argument("--backbone", metavar = "name", action = "store", default = 'vgg16', help = "Backbone network for feature extraction (vgg16, vgg19, resnet50, resnet101, resnet152)")
  parser.add_argument("--annot-dir", metavar = "name", action = "store", default = "Annotations", help = "Annotation directory")
  parser.add_argument("--wtrain-split", metavar = "name", action = "store", default = "wtrain", help = "Dataset split to use for unlabeled training samples")
  parser.add_argument("--results-to", metavar = "file", action = "store", help = "Save evaluation to text file (for many runs)")
  parser.add_argument("--run-number", metavar = "value", action = "store", help = "Number of the run (for many runs)")
  ...
  if not options.load_from:
    model.load_backbone_weights(weights_from=None)
    print("Initialized backbone layers to random weights")
  elif options.load_from == 'imagenet':
    model.load_backbone_weights(weights_from='imagenet')
    print("Initialized backbone layers to Keras ImageNet weights")
  elif options.load_from == 'vgg16places365':
    model.load_backbone_weights(weights_from='./places365_weights/vgg16-places365_weights_tf_dim_ordering_tf_kernels_notop.h5')
    print("Initialized backbone layers to Keras Places365 weights")
  else:
  ...
  elif options.self_train:
    selftrain(model = model, split = options.wtrain_split)
