...
class FasterRCNNModel(tf.keras.Model):
  def __init__(self, num_classes, allow_edge_proposals, custom_roi_pool, activate_class_outputs, l2 = 0, dropout_probability = 0, backbone = 'vgg16'):
    super().__init__()
    self._num_classes = num_classes
    self._activate_class_outputs = activate_class_outputs
    self._backbone = backbone
    self._stage1_feature_extractor = backbones.BackboneFeatureExtractor(name = backbone, l2 = l2, partially_trainable=False)

    print("Trainable state of model layers")
    for i, layer in enumerate(self._stage1_feature_extractor.layers):
  ...
  def call(self, inputs, training = False):
    ...
    feature_map = self._stage1_feature_extractor(input_image)
  ...
  def backbone_feature_pixels(self):
    return backbones.BackboneFeaturePixels(self._backbone)
  ...
  def load_backbone_weights(self, weights_from='imagenet'):
    """
    ...
    """
    keras_model = backbones.Predictor(self._backbone, weights_from)
    for keras_layer in keras_model.layers:
      weights = keras_layer.get_weights()
      if len(weights) > 0:
        backbone_layers = self._stage1_feature_extractor.layers + self._stage3_detector_network.layers
        our_layer = [ layer for layer in backbone_layers if layer.name == keras_layer.name ]
        if len(our_layer) > 0:
          print("Loading %s %s weights into layer: %s" % (self._backbone, weights_from, our_layer[0].name))
