...
def get_annotations(dataset_dir, annot_dir, split):
    classes_count = {}
    class_mapping = {}

    # pascal voc directory + 2012
    annot_path = os.path.join(dataset_dir, annot_dir)

    split_path = os.path.join(dataset_dir, 'ImageSets')
    split_path = os.path.join(split_path, 'Main')
    split_path = os.path.join(split_path, split + '.txt')
    split_file = open(split_path, "r")
    split_annot = split_file.read()
    split_annot = split_annot.split('\n')
    split_file.close()
    ...
    annots = tqdm(annots)
    for annot in annots:
        base_annot = annot.split(os.sep)[-1].split('.')[0]
        if not base_annot in split_annot:
            continue
        # try:
...
def get_classes_from(dataset_dir, annot_dir, split):
    # imageset_dir = os.path.join(dataset_dir, imageset, "Main")
    # classes = set(
    ...
    count, classes = get_annotations(dataset_dir, annot_dir, split)
    assert len(classes) > 0, "No classes found in %s" % annot_dir
    class_index_to_name = {(1 + v[0]): v[1] for v in enumerate(sorted(classes))}
    class_index_to_name[0] = "background"
    print('Samples per class')
    for ci in class_index_to_name:
        if ci > 0:
            print('{} -> {}'.format(class_index_to_name[ci], count[class_index_to_name[ci]]))
    return class_index_to_name


def get_class_mappings(dataset_dir, filepath):
    basename = os.path.splitext(os.path.basename(filepath))[0]
    mapping_file = os.path.join(dataset_dir, "Mappings", basename) + ".xml"
    tree = ET.parse(mapping_file)
    root = tree.getroot()
    assert tree != None, "Failed to parse %s" % mapping_file
    rules = {}
    for cls in root.findall("classes"):
        for c in cls.findall("class"):
            assert len(c.findall("name")) == 1
            class_name = c.find("name").text
            objects = c.find("objects")
            assert len(objects.findall("object")) >= 1
            objlist = []
            for obj in objects.findall("object"):
                objlist.append(obj.text)
            # corners = np.array([ y_min, x_min, y_max, x_max ]).astype(np.float32)
            rules[class_name] = objlist
    assert len(rules) > 0, "Mapping without classes %s" % mapping_file
    return rules
...
class Dataset:
  ...
  """
    ...
    def __init__(self, split, dir="gsv_vila_velha", annot_dir="Annotations", feature_pixels=16, augment=True, shuffle=True,
                 allow_difficult=False, cache=True, unannotated=False):
        """
    Parameters
    ----------
    split : str
    ...
    unannotated : bool
      Whether the training samples are annotated or not.
    """
        ...
        if not unannotated:
            ...
            self._gt_boxes_by_filepath = self._create_fake_boxes(filepaths=self._filepaths)
        self._i = 0
    ...
    def _get_classes(self):
        return get_classes_from(self._dir, self._annot_dir, self.split)
    ...
    def _get_ground_truth_boxes(self, filepaths, allow_difficult):
        ...
        for filepath in filepaths:
            basename = os.path.splitext(os.path.basename(filepath))[0]
            annotation_file = os.path.join(self._dir, self._annot_dir, basename) + ".xml"
    ...
    def _create_fake_boxes(self, filepaths):
        gt_boxes_by_filepath = {}
        for filepath in filepaths:
            basename = os.path.splitext(os.path.basename(filepath))[0]
            boxes = []
            x_min = 0  # convert to 0-based pixel coordinates
            y_min = 0
            x_max = 1
            y_max = 1
            corners = np.array([y_min, x_min, y_max, x_max]).astype(np.float32)
    ...
    def join(self, dataset):
        self._filepaths += dataset._filepaths
        self._iterable_filepaths += dataset._iterable_filepaths
        self._gt_boxes_by_filepath.update(dataset._gt_boxes_by_filepath)
